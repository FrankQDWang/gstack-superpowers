...
if [ -z "${BRANCH:-}" ]; then
  if [ "$MODE" = "report" ]; then
    ...
    BRANCH="automation/weekly-curated-workflow-upstream-sync-apply-$RUN_TS"
  fi
fi
COMMIT_TITLE="chore: sync curated workflow upstreams"
...
const action = canApply
  ? (mode === "apply" ? "已执行本地 apply" : "建议批准后执行 apply")
  : "建议暂缓 apply";
const wrapperAdvice = normalWrapperChanges.length > 0
  ? "wrapper 不新增，只刷新现有引用和校验值，并保持阶段边界"
  : "wrapper 暂不需要变化";
console.log(
  `${action}：gstack 候选从 ${gstack.active_commit ?? "null"} 到 ${gstack.candidate_commit ?? "null"}，有 ${gstackChanged} 个跟踪文件变化；superpowers 候选 ${superpowers.candidate_commit ?? "null"}，变化 ${superpowersChanged} 个。建议 ${wrapperAdvice}；${readOnly} 继续只读，不暴露成可调用 wrapper。评估为 ${status}/${recommendation}，policy violations ${policyViolations.length}，风险线索 ${riskMarkers.length}。${mode === "apply" ? "本次只更新本地 main；push、PR、merge、release 需要单独明确批准。" : "如同意，请回复批准后再运行本地 apply。"}`,
);
NODE
}
...
ensure_source_repo_ready_for_local_apply() {
  local current_branch
  current_branch="$(git -C "$SOURCE_REPO" branch --show-current)"
  if [ "$current_branch" != "$BASE_BRANCH" ]; then
    finish blocked "Source repo must be on $BASE_BRANCH before local apply; currently on $current_branch."
    exit 2
  fi
  if [ -n "$(git -C "$SOURCE_REPO" status --porcelain)" ]; then
    finish blocked "Source repo has uncommitted changes; local apply requires a clean $BASE_BRANCH checkout."
    exit 2
  fi
}
...
if [ "$MODE" = "report" ]; then
  ...
  finish needs-user "Review report is ready; approval is required before local apply."
  exit 0
fi
...
if git -C "$WORKTREE_DIR" diff --cached --quiet; then
  finish success "No upstream sync changes to apply locally."
  exit 0
fi

run_in_worktree git commit -m "$COMMIT_TITLE" || {
  finish failed "Could not commit weekly sync changes."
  exit 1
}

ensure_source_repo_ready_for_local_apply
run git -C "$SOURCE_REPO" merge --ff-only "$BRANCH" || {
  finish failed "Could not fast-forward local $BASE_BRANCH to the applied upstream sync commit."
  exit 1
}

{
  printf -- '- Runner status: `success`.\n'
  printf -- '- Message: Weekly upstream sync was applied locally.\n'
  printf -- '- Source repo: `%s`.\n' "$SOURCE_REPO"
  printf -- '- Worktree: `%s`.\n' "$WORKTREE_DIR"
  printf -- '- Branch: `%s`.\n' "$BRANCH"
  printf -- '- Mode: `%s`.\n' "$MODE"
...
append_memory
cleanup_old_runner_state
log "Weekly sync applied locally to $BASE_BRANCH."
rmdir "$LOCK_DIR" 2>/dev/null || true
